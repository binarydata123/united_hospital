...
const loginUser = (data) => {
  return new Promise((resolve, reject) => {
    const req = scAxiosAdmin.request('/patient-login', {
      method: 'post',
      headers: {
        'Accept': 'application/json',
      },
...
const loggedPatientDetails = (data) => {
  return new Promise((resolve, reject) => {
    const req = scAxiosAdmin.request('/patient-info', {
      method: 'get',
      headers: {
          'Accept': 'application/json',
      },
...
class PatientLogin extends Component {
  ...
  handleChange = event => {
    ...
    this.validateForm();
    // console.log(event.target.maxLength === event.target.value.length  );
  ...
  if (this.validateForm()) {
      ...
      if(patient_id && password){
        ...
        //     startUserSession(token, patient_id, phone_number, search_selected_doc_id, schedule_date, apointment_id, doctor_name, doctor_fees, patientName, schedule_time, search_selected_doc_name, doctor_profile_avability);
        //     this.props.nextStep();
        //   } else {
  ...
  LoginPaitentId = (patient_id,password) =>{
    ...
    .then(res => {
      if (!!res.token) {
        ...
        loggedPatientDetails(data)
        .then(patient_details_res => {
          let token = '';
          let login_phone_number = patient_details_res.data.phone;
          let search_selected_doc_id = '';
          let schedule_date = '';
          let apointment_id = '';
          let doctor_name = '';
          let doctor_fees = '';
          let patientName = patient_details_res.data.username;
          let schedule_time = '';
          let search_selected_doc_name = '';
          let doctor_profile_avability = '';
          startUserSession(res.token, patient_id, login_phone_number, search_selected_doc_id, schedule_date, apointment_id, doctor_name, doctor_fees, patientName, schedule_time, search_selected_doc_name, doctor_profile_avability);
  ...
  //   .then(res => {
  //     if (!!res.token) {
  //       startUserSession(res[0].token, data.patient_id, login_phone_number, search_selected_doc_id, schedule_date, apointment_id, doctor_name, doctor_fees, patientName, schedule_time, search_selected_doc_name, doctor_profile_avability);
  //       this.setState({ signin_success: true});
          ...
          <div className="offcanvas-body">
            ...
            <div className="left-offcanvas">
              ...
              <form onSubmit={this.handleSubmit} id="loginform" className="needs-validation" noValidate>
                <input placeholder="Enter Patient I.D (Optional)" type="text" id="patient_id" name="patient_id" value={this.state.fields.patient_id} onChange={this.handleChange} autoComplete="off" className="form-control" maxLength="11"/>
                <span style={alertStyle}>{this.state.errors.patient_id}</span>
                <div className="mt-3">
                ...
                {/* <p><label className="dont">Have a patient I.D?<br /><button className="nav-link active" onClick={this.ShowPatienIdPopup} style={{"background": "none", "border": "none", "padding": "0px", "color": "rgb(251, 98, 1)"}} data-bs-dismiss="offcanvas" aria-label="Close">Register Patient Id.</button></label></p> */}
              </form>
            </div>
          </div>
        </div>
